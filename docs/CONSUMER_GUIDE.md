...
A ready PR from a trusted author targeting the repository's default branch is eligible for approval regardless of labels. The `automerge` label is required only by the merge workflow. The current PR author is evaluated independently of the event actor. Approval is tied to the current head; an old approval does not suppress review of a new commit.

Dependabot approval uses the caller's automatic `GITHUB_TOKEN` and the `github-actions[bot]` identity, without PAT secrets. Grant `pull-requests: write` in the caller and enable **Allow GitHub Actions to create and approve pull requests** in repository Actions settings. Select `pull_request` for Dependabot; the toolkit's own caller retains `pull_request_target` for other authors. The PR author determines the credential even when another actor triggers the event. Existing CODEOWNERS and review rules still apply.

For other authors, explicitly forward `BOT_PAT` and optional `APPROVAL_PAT`. Approval first checks the actual account behind `BOT_PAT`. It uses `APPROVAL_PAT` only when `BOT_PAT` is absent or belongs to the PR author; the selected account must be independent of the author. Invalid credentials fail explicitly rather than silently selecting another account. `BOT_PAT` requires repository write permission for merging and pull request write permission for approval. Missing credentials or self-approval are configuration errors. The merge workflow continues using `BOT_PAT` for every author so downstream workflows retain their existing event behavior.
